...
print(
    "Let me log in (user doesn't exist):",
    requests.post(
        "http://localhost:8000/user/login",
        json={"email": "idontexist@ufl.edu", "password": "Nothing"},
    ).json(),
)
...
token = requests.post(
    "http://localhost:8000/user/signup", json=new_user.model_dump()
).json()["User Token"]
print(token)

# {'detail': 'Not authenticated'}
print(
    "Am I logged in? (Yes, but I didn't pass the token):",
    requests.get("http://localhost:8000/amiloggedin").json(),
)

# {'Success': 'You are logged in!'}
print(
    "Am I logged in? (Yes, and I passed the token):",
    requests.get(
...
print(
    "Let me log in (wrong password):",
    requests.post(
        "http://localhost:8000/user/login",
        json={"email": "johndoe@ufl.edu", "password": "WRONG"},
    ).json(),
)

token = requests.post(
    "http://localhost:8000/user/login",
    json={"email": "johndoe@ufl.edu", "password": "abc123"},
).json()["User Token"]
...
print(
    "Am I an admin? (no): ",
    requests.get(
        "http://localhost:8000/amianadmin", headers={"Authorization": f"Bearer {token}"}
    ).json(),
)
