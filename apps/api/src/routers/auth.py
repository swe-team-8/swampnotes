...
async def me(current: TokenUser | None = Depends(optional_user)):
    ...
    return {
        "user": {"sub": current.get("sub"), "email": current.get("email"), "username": current.get("username")},
        "message": "Signed in.",
    }
...
async def whoami(current: TokenUser = Depends(require_user)):
    return {"sub": current["sub"], "email": current.get("email"), "username": current.get("username")}
